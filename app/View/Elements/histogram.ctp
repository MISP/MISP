...
<table>
    ...
    ?>
                    <div class="attributehistogram-legend-line">
                        ...
                        <div style="display: inline-block;cursor: pointer;<?php if (in_array($type, $selectedTypes)) echo 'font-weight:bold';?>" role="button" tabindex="0" aria-label="<?php echo __('Toggle histogram');?>" title="<?php echo __('Toggle histogram');?>" onClick='toggleHistogramType("<?php echo h($type); ?>", <?php echo json_encode($selectedTypes, JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP); ?>);'><?php echo h($type);?></div>
                    </div>
    <?php
                    $cnt++;
                endif;
