...
class RolesController extends AppController {
	...
	public function admin_set_default($role_id = false) {
		if (!is_numeric($role_id) && $role_id !== false) {
			return new Response(array('body'=> json_encode(array('saved' => false, 'errors' => 'Invalid role.')),'status'=>200));
		}
		$this->loadModel('AdminSetting');
		$result = $this->AdminSetting->changeSetting('default_role', $role_id);
		if ($result === true) {
			return new Response(array('body'=> json_encode(array('saved' => true, 'success' => $role_id ? 'Default role set.' : 'Default role unset.')),'status'=>200));
		} else {
			return new Response(array('body'=> json_encode(array('saved' => false, 'errors' => $result)),'status'=>200));
		}
	}
}
