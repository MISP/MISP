<?php
    ...
    if ($isSiteAdmin) {
        ...
        if ($hasUnkwownCustomClusters) {
            ...
            $tagSampleHTML = sprintf('<li>%s</li>', implode('</li><li>', array_map('h', $unkownClustersDetails['unknownCustomClustersSamples'])));
        ...
        if ($hasUnkwownDefaultClusters) {
            ...
            $tagSampleHTML = sprintf('<li>%s</li>', implode('</li><li>', array_map('h', $unkownClustersDetails['unknownDefaultClustersSamples'])));
            $content .= $tagSampleHTML;
            $notices[] = sprintf('<div class="alert alert-%s" style="max-width: 960px; margin-bottom: 0.25em;"><b style="font-size: larger;">%s:</b>%s</div>', $severity, __('Info'), $content);
        }
        if (!empty($notices)) {
