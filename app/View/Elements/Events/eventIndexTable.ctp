<table class="table table-striped table-hover table-condensed">
    ...
    <tr id="event_<?= $eventId ?>">
        ...
        <?php endif; ?>
        <td class="short">
            <span><a href="<?= $baseurl."/events/view/".$eventId ?>" class="dblclickActionElement threat-level-<?= strtolower(h($event['ThreatLevel']['name'])) ?>" title="<?= __('Threat level: %s', h($event['ThreatLevel']['name'])) ?>"><?= $eventId ?></a> <?= $event['Event']['protected'] ? sprintf('<i class="green fas fa-lock" title="%s"></i>', __('Protected event')) : ''?></span>
        </td>
