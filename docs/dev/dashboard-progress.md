...
- [/] Implement remaining canonical types from PRD §5.5 — adapter `translate<Type>()` method + per-type PHPUnit coverage + JS picker + per-widget `$schema` backfills:
  ...
  - [/] `tag_filter` — **adapter + 18 PHPUnit tests landed 2026-05-19** in this commit. Canonical → legacy expansion writes top-level `include` / `exclude` keys (mirrors `date_range`'s 1-to-N pattern). Empty canonical lists do not overwrite legacy entries — user's bottom-tier-set legacy values survive a canonical-unset state. taxonomies / match_event_tags / match_attribute_tags canonical fields ship in `config['tag_filter']` for forward-compat but no translation runs (no legacy consumer today). JS chip-input picker + TrendingTagsWidget schema backfill land as separate follow-up commits.
