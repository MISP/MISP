		...
		foreach ($tags as $tag): ?>
			...
			<?php if ($isAclTagger): ?>
				<td style="padding-left:0px;padding-right:5px;">
					<?php
						echo $this->Form->create('Event', array('id' => 'removeTag_' . $tag['Tag']['id'],  'url' => '/events/removeTag/' . $event['Event']['id'] . '/' . $tag['Tag']['id'], 'style' => 'margin:0px;'));
					?>
					<span class="tagSecondHalf useCursorPointer noPrint" onClick="removeEventTag('<?php echo $event['Event']['id']; ?>', '<?php echo $tag['Tag']['id']; ?>');">x</span>
					<?php 
						echo $this->Form->end();
					?>
				</td>
