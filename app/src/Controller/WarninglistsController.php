...
class WarninglistsController extends AppController {
	...
	public function toggleEnable() {
		...
		if (!is_numeric($id)) return new Response(array('body'=> json_encode(array('saved' => false, 'errors' => 'Warninglist not found.')), 'status' => 200));
		$currentState = $this->Warninglist->find('first', array('conditions' => array('id' => $id), 'recursive' => -1));
		if (empty($currentState)) return new Response(array('body'=> json_encode(array('saved' => false, 'errors' => 'Warninglist not found.')), 'status' => 200));
		...
		if ($this->Warninglist->save($currentState)) {
			...
			return new Response(array('body'=> json_encode(array('saved' => true, 'success' => 'Warninglist ' . $message)), 'status' => 200));
		} else {
			return new Response(array('body'=> json_encode(array('saved' => false, 'errors' => 'Warninglist could not be enabled.')), 'status' => 200));
		}
	}
