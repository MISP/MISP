...
- [x] "Add Event → From Template" primary button on events index (both themes) — default theme shipped Phase 2.4 (`12e4ae647`); Overmind parity wired by rendering the existing `templatePickerModal` partial and injecting a sibling button into the `headerSection` BS5 action area via a small inline script (kept additive — no edit to the shared `headerSection.ctp` element). Gated on `eventTemplates/index` + `eventTemplates/instantiate` ACL.
...
- [x] ~~Events created from a template record `template_uuid` + `template_version` in event metadata~~ — **decided 2026-04-25: do not persist on the event.** Once the event is created, the originating template is no longer load-bearing for any user-facing flow we ship in v1. The audit-log row written by the next checkbox captures the template uuid + version for the rare audit-trail need; that is sufficient. This avoids tag pollution, a migration on `events`, and any REST/sync contract change. If a future analytics or filter use case demands persistence, it can be added without breaking existing data (a column or tag added later attaches only to events created after that point).
- [x] Audit log entry on "event created from template" (PRD §5.3 F3.4) — `EventTemplateInstantiator` writes a post-commit `instantiate` audit-log row tying the new event back to the source template (model=`EventTemplate`, model_id=template id, event_id=new event, change={uuid, version, name, event_uuid}). New `AuditLog::ACTION_INSTANTIATE = 'instantiate'` constant + `AuditLogsController` label map entries for clean rendering and dropdown-filterability. Audit failures are demoted to a CakeLog warning so they never block the committed event. Verified end-to-end against the live instance (template 463 → event 6699 → audit row 5295774).
