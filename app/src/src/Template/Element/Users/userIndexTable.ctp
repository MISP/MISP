	...
	<?php
		foreach ($users as $user): ?>
			<tr <?php echo $user['User']['disabled'] ? 'class="deleted_row"' : '';?>>
				<td class="short" ondblclick="document.location ='<?php echo $this->Url->build(array('admin' => true, 'action' => 'view', $user['User']['id']), true);?>';">
				...
				<td ondblclick="document.location ='<?php echo $this->Url->build(array('admin' => true, 'action' => 'view', $user['User']['id']), true);?>';">
					<?php echo h($user['User']['email']); ?>&nbsp;
				</td>
				<td ondblclick="document.location ='<?php echo $this->Url->build(array('admin' => true, 'action' => 'view', $user['User']['id']), true);?>';" class="quickSelect <?php echo $user['Role']['perm_auth'] ? 'bold' : 'grey'; ?>">
				...
				<td class="short" ondblclick="document.location ='<?php echo $this->Url->build(array('admin' => true, 'action' => 'view', $user['User']['id']), true);?>';" title="<?php echo !$user['User']['current_login'] ? 'N/A' : h(date("Y-m-d H:i:s",$user['User']['current_login']));?>">
				...
				<?php
					if (Configure::read('Plugin.CustomAuth_enable') && !Configure::read('Plugin.CustomAuth_required')):
				?>
					<td class="short" ondblclick="document.location ='<?php echo $this->Url->build(array('admin' => true, 'action' => 'view', $user['User']['id']), true);?>';" title="">
						<?php echo ($user['User']['external_auth_required'] ? 'Yes' : 'No'); ?>
					</td>
				<?php
					endif;
				?>
				<td class="short <?php if ($user['User']['disabled']) echo 'red bold';?>" ondblclick="document.location ='<?php echo $this->Url->build(array('admin' => true, 'action' => 'view', $user['User']['id']), true);?>';">
				...
				<td class="short action-links">
					...
					<?php
							...
							echo $this->Form->postLink('', array('admin' => true, 'action' => 'delete', $user['User']['id']), array('class' => 'icon-trash', 'title' => 'Delete'), __('Are you sure you want to delete # {0}? It is highly recommended to never delete users but to disable them instead.', $user['User']['id']));
						endif;
					?>
