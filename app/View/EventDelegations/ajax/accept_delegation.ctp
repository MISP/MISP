...
<p><?php echo __('Are you sure you would like to accept the request by %s to take ownership of Event #%s', h($delegationRequest['RequesterOrg']['name']), h($delegationRequest['Event']['id']));?>?</p>
		<table>
			<tr>
				...
				<td style="vertical-align:top;">
					<span title="<?php echo __('Cancel');?>" role="button" tabindex="0" aria-label="<?php echo __('Cancel');?>" class="btn btn-dark" id="PromptNoButton" onClick="cancelPrompt();"><?php echo __('No');?></span>
				</td>
			</tr>
		</table>
	</div>
