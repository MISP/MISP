...
class ServersController extends AppController {
	...
	public function serverSettingsEdit($setting, $id, $forceSave = false) {
		...
		if ($this->request->is('post')) {
			if (trim($this->request->data['Server']['value']) === '*****') {
				return new Response(array('body'=> json_encode(array('saved' => false, 'errors' => 'No change.')),'status'=>200));
			}
			...
			if (isset($found['beforeHook'])) {
				...
				if ($beforeResult !== true) {
					...
					return new Response(array('body'=> json_encode(array('saved' => false, 'errors' => $beforeResult)),'status'=>200));
				}
			}
			...
			if (!$forceSave && $testResult !== true) {
				...
				return new Response(array('body'=> json_encode(array('saved' => false, 'errors' => $errorMessage)),'status'=>200));
			} else {
				...
				if (isset($found['afterHook'])) {
					...
					if ($afterResult !== true) {
						...
						return new Response(array('body'=> json_encode(array('saved' => false, 'errors' => $afterResult)),'status'=>200));
					}
				}
				return new Response(array('body'=> json_encode(array('saved' => true, 'success' => 'Field updated.')),'status'=>200));
			}
		}
	}
	...
	public function postTest() {
		if ($this->request->is('post')) {
			...
			return new Response(array('body'=> json_encode($result)));
		} else {
	...
	public function testConnection($id = false) {
			...
			if ($result['status'] == 1) {
				...
				if (isset($version['version']) && preg_match('/^[0-9]+\.+[0-9]+\.[0-9]+$/', $version['version'])) {
					...
					return new Response(array('body'=> json_encode(array('status' => 1, 'local_version' => implode('.', $local_version), 'version' => implode('.', $version), 'mismatch' => $mismatch, 'newer' => $newer, 'post' => isset($post) ? $post : 'too old'))));
				} else {
					$result['status'] = 3;
				}
			}
	...
	public function startZeroMQServer() {
		...
		if ($result === true) return new Response(array('body'=> json_encode(array('saved' => true, 'success' => 'ZeroMQ server successfully started.')),'status'=>200));
	...
	public function stopZeroMQServer() {
		...
		if ($result === true) return new Response(array('body'=> json_encode(array('saved' => true, 'success' => 'ZeroMQ server successfully killed.')),'status'=>200));
		else return new Response(array('body'=> json_encode(array('saved' => false, 'errors' => 'Could not kill the previous instance of the ZeroMQ script.')),'status'=>200));
	}
	...
	public function update() {
		if ($this->request->is('post')) {
			...
			return new Response(array('body'=> $update));
		} else {
