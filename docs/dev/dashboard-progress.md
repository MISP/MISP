...
- [/] Implement remaining canonical types from PRD §5.5 — adapter `translate<Type>()` method + per-type PHPUnit coverage + JS picker + per-widget `$schema` backfills:
  ...
  - [/] `org_meta_filter` — **new canonical type added 2026-05-19** to PRD §5.5 + `WidgetSchema::CANONICAL_TYPES` + `TOOLBAR_ELIGIBLE_TYPES`. Shape: `{ sector?, type?, nationality?, name?, uuid?, local? }` where each string entry may be `!`-prefix-negated. Adapter case is **pass-through** — canonical and legacy widget shapes match, so the schema declaration alone is sufficient to wire the toolbar and the configure form picker. 8 in-tree widgets consume this shape today via the legacy slot `filter` (or `org_filter` for TrendingAttributesWidget, which uses `national` instead of `nationality` — flagged as separate Discovered work). 9 new PHPUnit tests on `translateOrgMetaFilter()` cover the pass-through contract (array / negation-prefix / unknown-keys / empty / null / non-array / under-different-schema-keys / coexists-with-time_window). **Adapter + tests landed 2026-05-19** (commit `ad0b9af94`). **JS picker landed 2026-05-19** (this commit): `app/webroot/js/dashboard/canonical/org_meta_filter.mjs` (~135 lines) exports `KEY` / `LABEL` / `buildField` / `readValue` / `displayLabel` / `equal`. The picker renders 6 chip-input rows (sector, type, nationality, name, uuid, local) using the same `buildChips()` from `chips.module.mjs` already in use by tag_filter. `readValue` returns a **sparse** object — only keys with at least one chip appear in the result, matching the "unset = field not present" convention the legacy widget's `!empty()`-guard expects. `equal` is a custom canonicaliser that treats empty arrays as equivalent to missing keys for mixed-state detection (default JSON.stringify would mismatch them). `displayLabel` emits compact summaries: `(none)` / `sector:2` / `sector:2, type:1` / `4 keys`. Wired into the configure form's `CANONICAL_BUILDERS` registry and the toolbar's `CANONICAL_REGISTRY`; CSS in dashboard.default.css adds a shared `.misp-org-meta-row` rule that piggybacks on the tag_filter row pattern (a 90px label column + flex-1 chip column for both). **8-widget backfills land as separate follow-up commits** (6 full-set widgets in one bundled commit, 2 local-set widgets in another).
  - [ ] `sharing_group_filter` — Phase 3 future.
  - [ ] `galaxy_cluster_filter` — Phase 3 future.
  - [ ] `distribution_filter` — Phase 3 future.
  - [ ] `threat_level_filter` — Phase 3 future.
  - [ ] `analysis_filter` — Phase 3 future.
