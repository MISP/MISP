...
class ServersTable extends AppTable
{
    ...
    public function pull(array $user, $technique, array $server, $jobId = false, $force = false)
    {
        ...
        if (!empty($eventIds)) {
            // download each event
            if ($jobId) {
                $JobsTable->saveProgress($jobId, __n('Pulling %s event.', 'Pulling %s events.', count($eventIds), count($eventIds)));
            }
    ...
    public function push($id, $technique, $jobId, $HttpSocket, array $user)
    {
        ...
        if ($jobId) {
            $JobsTable->saveStatus($jobId, true, __('Push to server %s complete.', $id));
        } else {
        ...
        return [
            'MISP' => [
                ...
                'log_user_ips_authkeys' => [
                    'level' => Server::SETTING_RECOMMENDED,
            ...
            'Security' => [
                ...
                'encryption_key' => [
                    ...
                    'afterHook' => function ($setting, $new, $old) {
                        /** @var SystemSetting $systemSetting */
                        $systemSettingsTable = $this->fetchTable('SystemSettings');
                        $systemSettingsTable->reencrypt($old, $new);

                        $this->reencryptAuthKeys($old, $new);

                        /** @var Cerebrate $cerebrate */
                        $CerebratesTable = $this->fetchTable('Cerebrates');
                        $CerebratesTable->reencryptAuthKeys($old, $new);
                        return true;
                    },
                    'type' => 'string',
                    'null' => true,
