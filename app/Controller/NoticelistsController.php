...
class NoticelistsController extends AppController {
	...
	public function toggleEnable() {
		...
		if (!is_numeric($id)) return new CakeResponse(array('body'=> json_encode(array('saved' => false, 'errors' => 'Noticelist not found.')), 'status' => 200, 'type' => 'json'));
		$currentState = $this->Noticelist->find('first', array('conditions' => array('id' => $id), 'recursive' => -1));
		if (empty($currentState)) return new CakeResponse(array('body'=> json_encode(array('saved' => false, 'errors' => 'Noticelist not found.')), 'status' => 200, 'type' => 'json'));
	...
	public function enableNoticelist($id, $enable = false) {
		...
		if (!$this->Noticelist->exists()) throw new NotFoundException('Invalid Noticelist.');
