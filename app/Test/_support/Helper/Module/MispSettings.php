...
final class MispSettings extends \Codeception\Module implements \Codeception\Lib\Interfaces\DependsOnModule
{
    ...
    {
        if (isset($this->config['docker_misp_dir'])) {
            if (!isset($this->config['docker_misp_dir'])) {
                throw new Exception('Config key `docker_misp_dir` must be set when running in docker mode.');
            } else {
                $cmd = sprintf(
                    'docker-compose -f %s/docker-compose.yml exec -T --user www-data misp bash -c "/var/www/MISP/app/Console/cake Admin setSetting %s %s"',
                    $this->config['docker_misp_dir'],
                    $setting,
                    $value
                );
            }
        ...
        try {
            ...
            sleep(2); // FIXME: otherwise sometimes the config change does not reflect
        } catch (Exception $ex) {
            throw new Exception(
