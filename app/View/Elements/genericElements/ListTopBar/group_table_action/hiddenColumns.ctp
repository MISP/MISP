...
foreach ($table_data['fields'] as $field) {
    ...
    $fieldName = !empty($field['name']) ? $field['name'] : \Inflector::humanize($field['data_path']);
    $isVisible = !in_array(h(\Inflector::variable($fieldName)), $tableSettings['hidden_column']);
    ...
    $availableColumnsHtml .= sprintf(
        ...
        h(\Inflector::variable($fieldName)),
        h(\Inflector::variable($fieldName)),
        $isVisible ? 'checked' : '',
        h(\Inflector::variable($fieldName)),
        h($fieldName)
