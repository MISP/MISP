...
class SyncTool
{
    ...
    public static function getServerClientCertificateInfo(array $server)
    {
        ...
        $clientCertificate = new \SplFileObject(APP . "files" . DS . "certs" . DS . $server['id'] . '_client.pem');
        if (!$clientCertificate->isFile()) {
            throw new Exception("Certificate file '{$clientCertificate->getPath()}' doesn't exists.");
        }

        $certificateContent = $clientCertificate->fread($clientCertificate->getSize());
        if ($certificateContent === false) {
            throw new Exception("Could not read '{$clientCertificate->getPath()}' file with client certificate.");
        }
