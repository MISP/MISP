...
/**
 * The historic MISP database update corpus, frozen.
 *
 * Every statement in here has run successfully on instances in the wild for over
 * a decade, and nothing tests it. It was moved out of AppModel verbatim - not one
 * line of its SQL was rewritten - and it is not ported to the new migration DSL.
 * Treat it as an archive: it exists so that an instance at any historic db_version
 * can still be brought forward, and it should only ever be read, never edited.
 *
 * New schema changes do NOT belong here. db_version is frozen at 159; anything
 * beyond that is a migration class under app/Lib/Migration/Migrations/.
 *
 * This is a trait rather than a class on purpose. The two switch bodies contain
 * 160 $this-> bindings across 34 distinct members, 12 of them private methods on
 * AppModel. A trait keeps $this-> resolving to the model, which made the move a
 * literal cut and paste with zero edits and zero visibility changes. self:: in a
 * trait method resolves to the composing class, so the DB_CHANGES /
 * OLD_DB_CHANGES / ADVANCED_UPDATES_DESCRIPTION constants stay on AppModel.
 */
trait LegacyMigrationsTrait
{
