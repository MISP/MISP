...
class UsersController extends AppController {
	...
	public function request_API(){
		if (Configure::read('MISP.disable_emailing')) {
			return new Response(array('body'=> json_encode(array('saved' => false, 'errors' => 'API access request failed. E-mailing is currently disabled on this instance.')),'status'=>200));
		}
		...
		if (isset($responsibleAdmin['email']) && !empty($responsibleAdmin['email'])) {
			...
			if ($result) {
				return new Response(array('body'=> json_encode(array('saved' => true, 'success' => 'API access requested.')),'status'=>200));
			}
		}
		return new Response(array('body'=> json_encode(array('saved' => false, 'errors' => 'Something went wrong, please try again later.')),'status'=>200));
	}
	...
	public function login() {
		...
		if ($this->Auth->login()) {
			...
			$dataSourceConfig = ConnectionManager::get('default')->config;
	...
	public function initiatePasswordReset($id, $firstTime = false) {
		...
		if ($this->request->is('post')) {
			...
			return new Response($this->User->initiatePasswordReset($user, $firstTime));
		} else {
			$error = false;
			$encryption = false;
			if (!empty($user['User']['gpgkey'])) {
