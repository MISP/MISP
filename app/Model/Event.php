...
class Event extends AppModel
{
    ...
    private function cleanupEventArrayFromXML(&$data)
    {
        ...
        foreach ($objects as $object) {
            ...
            if (isset($data['Event'][$object]) && is_array($data['Event'][$object]) && count($data['Event'][$object])) {
                if (!array_is_list($data['Event'][$object])) {
                    // single attribute
    ...
    /**
     * Filter massaging shared by the attribute and object level restSearch.
     * It deliberately differs from restSearchFilterMassage(): there is no
     * tag -> tags aliasing and no withAttachments -> includeAttachments
     * mapping, the value filter is dropped once it became a wildcard and the
     * ignore filter is handled in buildFilterConditions() instead.
     *
     * @param array $filters
     * @param bool $non_restrictive_export
     * @param array $user
     * @return array
     */
    public function restSearchFilterMassageAttributeLevel($filters, $non_restrictive_export, $user)
    {
        if (empty($non_restrictive_export)) {
            if (!isset($filters['to_ids'])) {
                $filters['to_ids'] = 1;
    ...
    /**
     * The fetchAttributes()/fetchObjects() include flags that the attribute
     * and object level restSearch endpoints pass through identically.
     *
     * @param array $filters
     * @return array
     */
    public function restSearchIncludeFlags($filters)
    {
        return array(
