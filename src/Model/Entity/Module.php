...
class Module extends AppModel
{
    ...
    private const TYPE_TO_FAMILY = array(
        'Import' => 'Import',
        'Export' => 'Export',
        'Action' => 'Action',
        'hover' => 'Enrichment',
    ...
    private function __getModuleServer($moduleFamily = 'Enrichment')
    {
        ...
        if (empty($url) || empty($port)) {
            ...
            if (empty($url)) {
                $url = $ServersTable->serverSettings['Plugin'][$moduleFamily . '_services_url']['value'];
            }
            if (empty($port)) {
                $port = $ServersTable->serverSettings['Plugin'][$moduleFamily . '_services_port']['value'];
            }
        }
    ...
    private function __prepareAndExecuteTrigger($postData, $triggerData = []): bool
    {
        ...
        if (!$WorkflowsTable->isTriggerCallable($trigger_id)) {
            return true;
        }
        if (empty($triggerData) && !empty($postData['attribute_uuid'])) {
            $UsersTable = $this->fetchTable('Users');
            $AttributesTable = $this->fetchTable('Attributes');
            $user = $UsersTable->getAuthUser(Configure::read('CurrentUserId'), true);
            $options = [
                'conditions' => [
            ...
            $attributes = $AttributesTable->fetchAttributes($user, $options);
            ...
            $event = $EventsTable->quickFetchEvent($postData['event_id']);
            $triggerData = $event;
            ...
            if (isset($triggerData['Attribute'])) {
                $logging['message'] = __(
                    'The workflow `%s` prevented attribute `%s` (from event `%s`) to query the module `%s`',
                    $trigger_id,
