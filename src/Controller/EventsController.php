...
class EventsController extends AppController
{
    ...
    public function filterEventIndex()
    {
        ...
        $sharingGroups = $this->Events->SharingGroup->fetchAllAuthorised($this->ACL->getUser(), 'name', true);
    ...
    public function add()
    {
        ...
        // $sgs = $this->Events->SharingGroup->fetchAllAuthorised($this->ACL->getUser(), 'name', 1);
            ...
            if (!empty($this->request->getData())) {
                ...
                if ($add === true) {
                    if ($this->ParamHandler->isRest()) {
                        ...
                        $results = $this->Events->fetchEvent($this->ACL->getUser(), ['eventid' => $created_id, 'metadata' => $metadata]);
                        $event = $results[0];
        ...
        if ($this->request->is('post') || $this->request->is('put')) {
            if ($this->ParamHandler->isRest()) {
                ...
                if ($result === true) {
                    ...
                    $results = $this->Events->fetchEvent($this->ACL->getUser(), ['eventid' => $id, 'metadata' => $metadata]);
                    $event = $results[0];
                    return $this->__restResponse($event);
                } else {
                    $message = 'Error';
