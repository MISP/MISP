...
<div class="btn-group">
    ...
    <div class="dropdown-menu dropdown-menu-end">
        <h6 class="dropdown-header">
            ...
            <div>
                <?= $this->SocialProvider->getIcon($me) ?>
                <div class="ms-1 me-3">
                    <strong><?= h($me['email']) ?></strong>
                </div>
                <div class="ms-1 me-3 d-flex">
                    [<?= h($me['Organisation']['name']) ?>]
                    <span class="ms-auto"><?= h($me['Role']['name']) ?></span>
                </div>
            </div>
        </h6>
        <div class="dropdown-divider"></div>
        <a class="dropdown-item" href="<?= Router::url(['controller' => 'users', 'action' => 'view', 'plugin' => null, h($me['id'])]) ?>">
            <i class="me-1 <?= $this->FontAwesome->getClass('user-circle') ?>"></i>
            <?= __('My Account') ?>
        </a>
        <a
            class="dropdown-item"
            href="<?= Router::url(['controller' => 'users', 'action' => 'settings', 'plugin' => null, h($me['id'])]) ?>"
        >
        ...
        <?php
        if (
            !empty($this->SocialProvider->hasSocialProfile($me)) &&
        ...
        >
            <?php if (!empty($this->SocialProvider->getIcon($me))): ?>
                <?= $this->SocialProvider->getIcon($me) ?>
            <?php else: ?>
