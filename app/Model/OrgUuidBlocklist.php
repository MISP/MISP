...
/**
 * Shared implementation of the blocklists that block on an organisation UUID.
 * The children only differ by the table they use and by the redis key prefix
 * under which their block counters are stored.
 */
abstract class OrgUuidBlocklist extends AppModel
{
    public $recursive = -1;
    ...
    /**
     * @var string Prefix of the redis keys holding the block counters
     */
    protected $blockedKeyPrefix = 'misp:blocklist_blocked';
    ...
    {
        parent::afterDelete();
        if (!empty($this->data[$this->alias]['org_uuid'])) {
            $this->cleanupBlockedCount($this->data[$this->alias]['org_uuid']);
        }
    }
    ...
    public function afterFind($results, $primary = false)
    {
        foreach ($results as $k => $result) {
            if (isset($result[$this->alias]['org_uuid'])) {
                $results[$k][$this->alias]['blocked_data'] = $this->getBlockedData($result[$this->alias]['org_uuid']);
            }
        }
        return $results;
    }
    ...
    public function removeBlockedEvents(array &$eventArray)
    {
        $blocklistHits = $this->find('column', array(
            'conditions' => array($this->alias . '.org_uuid' => array_unique(array_column($eventArray, 'orgc_uuid'))),
            'fields' => array($this->alias . '.org_uuid'),
        ));
    ...
    public function isBlocked($orgIdOrUuid)
    {
        ...
        $isBlocked = $this->hasAny([$this->alias . '.org_uuid' => $orgUuid]);
    ...
    public function saveEventBlocked($orgIdOrUUID)
    {
        ...
        $lastBlockTime = time();
        $redisKeyBlockAmount = "{$this->blockedKeyPrefix}_amount:{$orgcUUID}";
        $redisKeyBlockLastTime = "{$this->blockedKeyPrefix}_last_time:{$orgcUUID}";
