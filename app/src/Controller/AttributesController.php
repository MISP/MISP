...
class AttributesController extends AppController {
		...
		if ($this->request->is('post')) {
			...
			if ($this->_isRest()) {
				...
				if ($this->request->is('ajax')) {
					...
					if (!empty($successes)) {
						return new Response(array('body'=> json_encode(array('saved' => true, 'success' => $message)),'status' => 200));
					} else {
						return new Response(array('body'=> json_encode(array('saved' => false, 'errors' => $errors)),'status' => 200));
					}
				} else {
	...
	public function editField($id) {
		...
		$this->Attribute->id = $id;
		if (!$this->Attribute->exists()) {
			return new Response(array('body'=> json_encode(array('fail' => false, 'errors' => 'Invalid attribute')),'status'=>200));
		}
		$this->Attribute->recursive = -1;
		...
		foreach ($this->request->data['Attribute'] as $changedKey => $changedField) {
			...
			if ($attribute['Attribute'][$changedKey] == $changedField) {
				$this->autoRender = false;
				return new Response(array('body'=> json_encode('nochange'),'status'=>200));
			}
		...
		if ($this->Attribute->save($attribute)) {
			...
			$this->autoRender = false;
			return new Response(array('body'=> json_encode(array('saved' => true, 'success' => 'Field updated.', 'check_publish' => true)),'status'=>200));
		} else {
			$this->autoRender = false;
			return new Response(array('body'=> json_encode(array('saved' => false, 'errors' => $this->Attribute->validationErrors)),'status'=>200));
		}
	}
		...
		if ($this->request->is('ajax')) {
			if ($this->request->is('post')) {
				if ($this->__delete($id, $hard)) {
					return new Response(array('body'=> json_encode(array('saved' => true, 'success' => 'Attribute deleted.')),'status'=>200));
				} else {
					return new Response(array('body'=> json_encode(array('saved' => false, 'errors' => 'Attribute was not deleted.')),'status'=>200));
				}
			} else {
				$this->set('hard', $hard);
	...
	public function restore($id = null) {
		...
		if (empty($attribute) || !$this->userRole['perm_site_admin'] && $this->Auth->user('org_id') != $attribute['Event']['orgc_id']) {
			if ($this->request->is('ajax')) return new Response(array('body'=> json_encode(array('saved' => false, 'errors' => 'Invalid Attribute')),'status'=>200));
			else throw new MethodNotAllowedException('Invalid Attribute');
		}
		if ($this->request->is('ajax')) {
			if ($this->request->is('post')) {
				$result = $this->Attribute->restore($id, $this->Auth->user());
				if ($result === true) return new Response(array('body'=> json_encode(array('saved' => true, 'success' => 'Attribute restored.')),'status'=>200));
				else return new Response(array('body'=> json_encode(array('saved' => false, 'errors' => $result)),'status'=>200));
			} else {
				$this->set('id', $id);
				$this->set('event_id', $attribute['Attribute']['event_id']);
	...
	public function deleteSelected($id) {
		...
		if (count($fails) == 0 && count($successes) > 0) {
			return new Response(array('body'=> json_encode(array('saved' => true, 'success' => count($successes) . ' attribute' . (count($successes) != 1 ? 's' : '') . ' deleted.')),'status'=>200));
		} else {
			return new Response(array('body'=> json_encode(array('saved' => false, 'errors' => count($successes) . ' attribute' . (count($successes) != 1 ? 's' : '') . ' deleted, but ' . count($fails) . ' attribute' . (count($fails) != 1 ? 's' : '') . ' could not be deleted.')),'status'=>200));
		}
	}
	...
	public function editSelected($id) {
		...
		if ($this->request->is('post')) {
			...
			if ($this->request->data['Attribute']['to_ids'] == 2 && $this->request->data['Attribute']['distribution'] == 6 && $this->request->data['Attribute']['comment'] == null) {
				$this->autoRender = false;
				return new Response(array('body'=> json_encode(array('saved' => true)),'status' => 200));
			}
			...
			if ($this->Attribute->saveMany($attributes)) {
				...
				$this->autoRender = false;
				return new Response(array('body'=> json_encode(array('saved' => false)),'status' => 200));
			}
		} else {
	...
	public function attributeReplace($id) {
		...
		if ($this->request->is('post')) {
			...
			$this->autoRender = false;
			$this->layout = 'ajax';
			if ($success) return new Response(array('body'=> json_encode(array('saved' => true, 'success' => $message)),'status'=>200));
			else return new Response(array('body'=> json_encode(array('saved' => true, 'errors' => $message)),'status'=>200));
		}
	}
	...
	public function checkOrphanedAttributes() {
		...
		$events = array_keys($this->Attribute->Event->find('list'));
		$orphans = $this->Attribute->find('list', array('conditions' => array('Attribute.event_id !=' => $events)));
		return new Response(array('body'=> count($orphans), 'status'=>200));
	}
	...
	public function addTag($id = false, $tag_id = false) {
		if (!$this->request->is('post')) {
			return new Response(array('body'=> json_encode(array('saved' => false, 'errors' => 'You don\'t have permission to do that. Only POST requests are accepted.')), 'status' => 200));
		}
		...
		if (!is_numeric($tag_id)) {
			...
			if (empty($tag)) return new Response(array('body'=> json_encode(array('saved' => false, 'errors' => 'Invalid Tag.')), 'status' => 200));
			$tag_id = $tag['Tag']['id'];
		}
		if (!isset($idList)) {
			$idList = array($id);
		...
		foreach ($idList as $id) {
			...
			if (!$this->_isSiteAdmin() && !$this->userRole['perm_sync']) {
				if (!$this->userRole['perm_tagger'] || ($this->Auth->user('org_id') !== $event['Event']['org_id'] && $this->Auth->user('org_id') !== $event['Event']['orgc_id'])) {
					return new Response(array('body'=> json_encode(array('saved' => false, 'errors' => 'You don\'t have permission to do that.')), 'status' => 200));
				}
			}
			...
			if (!$this->Attribute->AttributeTag->Tag->exists()) {
				return new Response(array('body'=> json_encode(array('saved' => false, 'errors' => 'Invalid Tag.')), 'status' => 200));
			}
		...
		if ($fails == 0) {
			...
			return new Response(array('body'=> json_encode(array('saved' => true, 'success' => $message, 'check_publish' => true)), 'status' => 200));
		} else {
			if ($fails == 1) {
			...
			return new Response(array('body'=> json_encode(array('saved' => false, 'errors' => $message)), 'status' => 200));
		}
	}
	...
	public function removeTag($id = false, $tag_id = false) {
		if (!$this->request->is('post')) {
			...
			if ($this->Attribute->data['Attribute']['deleted']) throw new NotFoundException(__('Invalid attribute'));
			$eventId = $this->Attribute->data['Attribute']['event_id'];
			if (empty($tag_id)) return new Response(array('body'=> json_encode(array('saved' => false, 'errors' => 'Invalid Tag.')), 'status' => 200));
			if (!is_numeric($tag_id)) {
				$tag = $this->Attribute->AttributeTag->Tag->find('first', array('recursive' => -1, 'conditions' => array('LOWER(Tag.name) LIKE' => strtolower(trim($tag_id)))));
				if (empty($tag)) return new Response(array('body'=> json_encode(array('saved' => false, 'errors' => 'Invalid Tag.')), 'status' => 200));
				$tag_id = $tag['Tag']['id'];
			...
			$this->autoRender = false;
			if (empty($attributeTag)) return new Response(array('body'=> json_encode(array('saved' => false, 'errors' => 'Invalid attribute - tag combination.')), 'status' => 200));
			$tag = $this->Attribute->AttributeTag->Tag->find('first', array(
				'conditions' => array('Tag.id' => $tag_id),
				'recursive' => -1,
				'fields' => array('Tag.name')
			...
			if ($this->Attribute->AttributeTag->delete($attributeTag['AttributeTag']['id'])) {
				...
				return new Response(array('body'=> json_encode(array('saved' => true, 'success' => 'Tag removed.', 'check_publish' => true)), 'status' => 200));
			} else {
				return new Response(array('body'=> json_encode(array('saved' => false, 'errors' => 'Tag could not be removed.')), 'status' => 200));
			}
		}
	}
	...
	public function toggleCorrelation($id) {
		...
		if ($this->request->is('post')) {
			...
			if ($this->_isRest()) {
				...
				return new Response(array('body'=> json_encode(array('saved' => true, 'success' => ('Correlation ' . ($attribute['Attribute']['disable_correlation'] ? 'disabled' : 'enabled')), 'check_publish' => true)),'status'=>200));
			}
		} else {
	...
	public function checkAttachments() {
			...
			return new Response(array('body'=>$counter, 'status'=>200));
	}
}
