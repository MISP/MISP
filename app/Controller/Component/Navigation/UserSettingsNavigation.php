...
class UserSettingsNavigation extends BaseNavigation
{
    ...
    public function addLinks()
    {
        ...
        $this->bcf->addLink(
            ...
            'Users',
            'view',
            function ($config) use ($bcf, $request) {
            if (!empty($request->query('Users_id'))) {
                $user_id = h($request->query('Users_id'));
                $linkData = [
        ...
        if ($this->request->params['controller'] == 'UserSettings' && $this->request->params['action'] == 'index') {
            if (!empty($this->request->query('Users_id'))) {
                $user_id = $this->request->query('Users_id');
                $this->bcf->addParent(
                    'UserSettings',
                    'index',
                    'Users',
