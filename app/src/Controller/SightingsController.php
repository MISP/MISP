...
class SightingsController extends AppController {
		...
		if ($this->request->is('post')) {
			...
			if ($this->request->is('ajax')) {
				if ($error) {
					...
					return new Response(array('body'=> json_encode(array('saved' => false, 'errors' => $error_message)), 'status' => 200));
				} else {
					return new Response(array('body'=> json_encode(array('saved' => true, 'success' => $result . ' ' . $this->Sighting->type[$type] . (($result == 1) ? '' : 's') . '  added.')), 'status' => 200));
				}
			} else {
				if ($error) {
	...
	public function quickDelete($id, $rawId, $context) {
		...
		if (!$this->request->is('post')) {
			...
			if (!isset($id)) {
				return new Response(array('body'=> json_encode(array('saved' => true, 'errors' => 'Invalid request.')), 'status' => 200));
			} else {
				$sighting = $this->Sighting->find('first', array('conditions' => array('Sighting.id' => $id), 'recursive' => -1));
				if (empty($sighting)) {
					return new Response(array('body'=> json_encode(array('saved' => true, 'errors' => 'Invalid sighting.')), 'status' => 200));
				}
				...
				if ($result) {
					return new Response(array('body'=> json_encode(array('saved' => true, 'success' => 'Sighting deleted.')), 'status' => 200));
				} else {
					return new Response(array('body'=> json_encode(array('saved' => true, 'errors' => 'Sighting could not be deleted')), 'status' => 200));
				}
			}
		}
	}
