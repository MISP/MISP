...
class Event extends AppModel {
	...
	public function sendAlertEmail($id, $org, $processId = null) {
		...
		//
		if ($eventIsPrivate) {
			$conditions = array('User.autoalert' => 1, 'User.gpgkey =' => "", 'User.certif_public =' => "",  'User.org =' => $event['Event']['org']);
		} else {
			$conditions = array('User.autoalert' => 1, 'User.gpgkey =' => "", 'User.certif_public =' => "");
		}
		if (!Configure::read('GnuPG.onlyencrypted') and !Configure::read('SMIME.onlyencrypted')) {
		...
		//
		if ($eventIsPrivate) {
			$conditions = array('User.autoalert' => 1, 'User.org =' => $event['Event']['org']);
			//$conditions = array('User.autoalert' => 1, "OR" => array('User.gpgkey !=' => "", 'User.certif_public !=' => ""), 'User.org =' => $event['Event']['org']);
		} else {
			$conditions = array('User.autoalert' => 1);
			//$conditions = array('User.autoalert' => 1, "OR" => array('User.gpgkey !=' => "", 'User.certif_public !=' => ""));
		}
		$alertUsers = $this->User->find('all', array(
			'conditions' => $conditions,
			'recursive' => 0,
		...
		foreach ($alertUsers as $k => &$user) {
			...
			if ((Configure::read('GnuPG.onlyencrypted')) and ($user['User']['gpgkey'] != '')){
				try {
			...
			} elseif ((Configure::read('SMIME.onlyencrypted')) and ($user['User']['certif_public'] != '')){
				...
				$headers_smime = array("To" => $user['User']['email'], "From" => Configure::read('MISP.email'), "Subject" => "[" . Configure::read('MISP.org') . " MISP] Event " . $id . " - " . $subject . " - " . $event['ThreatLevel']['name'] . " - TLP Amber");
				// encrypt it
				...
				// LAST CHANCE to send the mail 
				// Create an email with the link of the event :)
				$body_link = "";
				$body_link .= "Hello, \n";
				$body_link .= "\n";
				$body_link .= "The event number " . $event['Event']['id'] . " was published on the MISP platform. \n";	
				$body_link .= "\n";
				$body_link .= "Since no GPG/PGP key or public certificate is set in your MISP profile, you can not directly receive the information in an encrypted email. \n";
				$body_link .= "You can fill out you profile with your GPG/PGP key or public to receive encrypted email. \n\n";
				$body_link .= "To access to the event involved, please click on the following link:\n";
				$body_link .= 'URL    : ' . Configure::read('MISP.baseurl') . '/events/view/' . $event['Event']['id'] . "\n";
				$body_link .= "\n\n";
				$body_link .= "Best regards,\n\n";
				$body_link .= Configure::read('MISP.org');
				// Send it
				$this->log('LAST CHANCE to send the mail -- '.$user['User']['email'].' -- sendAlertEmail', 'debug');
				$Email->send($body_link);
			}
	...
	public function sendContactEmail($id, $message, $all, $user, $isSiteAdmin) {
		...
		if (!$all) {
			...
			$temp = $this->User->findAllByOrg($event['Event']['org'], array('email', 'gpgkey', 'certif_public', 'contactalert', 'id'));
			...
			$orgMembers = $this->User->findAllById($event['Event']['user_id'], array('email', 'gpgkey', 'certif_public'));
		}
		...
		$body_link .= "Hello, \n";
		$body_link .= "\n";
		$body_link .= "Someone wants to get in touch with you concerning a MISP event. \n";
		$body_link .= "\n";
		$body_link .= "You can reach him at " . $user['User']['email'] . "\n";
		if (!$user['User']['gpgkey'])
		...
		if (Configure::read('GnuPG.onlyencrypted')){
			...
			if (openssl_pkcs7_sign($msg, $signed, 'file://'.Configure::read('SMIME.cert_public_sign'), array("file://".Configure::read('SMIME.key_sign'), Configure::read('SMIME.password')), array(), PKCS7_TEXT)){
				...
				$this->log('sign No -- sendContactEmail', 'debug');
				$this->log('SMIME.cert_public_sign', 'debug');
				$this->log(Configure::read('SMIME.cert_public_sign'), 'debug');
			}
		}
		foreach ($orgMembers as &$reporter) {
			...
			if (!empty($reporter['User']['gpgkey'])) {
				...
				fclose($fp);
				$msg_signed_encrypted = tempnam('/dev/shm/', 'SMIME');
				// Create the hearder (they have to be enbcrypted)
				$headers_smime = array("To" => $reporter['User']['email'], "From" => Configure::read('MISP.email'), "Subject" => "[" . Configure::read('MISP.org') . " MISP] Need info about event " . $id . " - TLP Amber");
				//$headers_smime = array('Content-Transfer-Encoding' => 'base64', 'Content-Type' => 'application/pkcs7-mime; name="smime.p7m"; smime-type=enveloped-data', 'Content-Disposition' => 'attachment; filename="smime.p7m"', 'Content-Description' => 'S/MIME Encrypted Message');
				// encrypt it
				if (openssl_pkcs7_encrypt($msg_signed, $msg_signed_encrypted, $certif_public, $headers_smime, 0, OPENSSL_CIPHER_AES_256_CBC)){
					...
					$this->log('encrypt No -- sendContactEmail', 'debug');
					$this->log('SMIME.cert_public_sign', 'debug');
					$this->log(Configure::read('SMIME.cert_public_sign'), 'debug');
				}
			} else {
			...
			if ($smime_encrypted) {
				...
				if ($user['User']['gpgkey'] != null) {
					// attach the gpg key
					$Email->attachments(array('gpgkey.asc' => $tmpfname));
				}
			}
			$Email->from(Configure::read('MISP.email'));
			$Email->replyTo($user['User']['email']);
