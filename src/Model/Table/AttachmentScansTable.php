...
class AttachmentScansTable extends AppTable
{
    /** @var AttachmentTool */
    ...
    public function diagnostic()
    {
        ...
        if (in_array('attachment', $moduleInfo['types'], true)) {
            $fakeAttribute = [
                ...
                'type' => 'attachment',
                'value' => 'eicar.com',
                'data' => AttachmentScan::EICAR,
            ];
        } else {
            $hashAlgo = $moduleInfo['types'][0];
            $hash = hash($hashAlgo, base64_decode(AttachmentScan::EICAR));
            $fakeAttribute = [
                'uuid' => Text::uuid(),
                'event_id' => 1,
                'type' => $hashAlgo,
    ...
    public function insertScan($type, $attributeId, $infected, $malwareName = null)
    {
        ...
        $attachmentScan = $this->newEntity(array(
            'type' => $type,
        ...
        $result = $this->save($attachmentScan);
        if (!$result) {
    ...
    public function scan($type, $attributeId = null, $jobId = null)
    {
        ...
        if ($jobId && !$JobsTable->exists($jobId)) {
        ...
        if ($type === 'all') {
            $attributes = $this->fetchTable('Attributes')->find('all', array(
                'recursive' => -1,
                'conditions' => ['type' => 'attachment'],
                'fields' => $fields,
            ));
            $shadowAttributes = $this->fetchTable('ShadowAttributes')->find('all', array(
                'recursive' => -1,
                'conditions' => ['type' => 'attachment'],
                'fields' => $fields,
            ));
            $attributes = array_merge($attributes, $shadowAttributes);
        } else if ($type === AttachmentScan::TYPE_ATTRIBUTE) {
            $attributes = $this->fetchTable('Attributes')->find('all', array(
                'recursive' => -1,
                'conditions' => ['type' => 'attachment', 'id' => $attributeId],
                'fields' => $fields,
            ));
        } else if ($type === AttachmentScan::TYPE_SHADOW_ATTRIBUTE) {
            $attributes = $this->fetchTable('ShadowAttributes')->find('all', array(
                'recursive' => -1,
                'conditions' => ['type' => 'attachment', 'id' => $attributeId],
                'fields' => $fields,
            ));
        ...
        try {
            ...
            $JobsTable->saveStatus($jobId, false, 'Could not connect to attachment scan module.');
        ...
        foreach ($attributes as $attribute) {
            $type = isset($attribute['Attribute']) ? AttachmentScan::TYPE_ATTRIBUTE : AttachmentScan::TYPE_SHADOW_ATTRIBUTE;
            try {
            ...
            $JobsTable->saveProgress($jobId, $message, ($scanned + $fails) / $attributes->count() * 100);
        }

        if ($scanned === 0 && $fails > 0) {
            $JobsTable->saveStatus($jobId, false);
            return false;
        } else {
            $message = "$scanned files scanned, $virusFound malware files found.";
            if ($fails) {
            ...
            $JobsTable->saveStatus($jobId, true, "Job done, $message");
            return $message;
        }
    }
    ...
    public function backgroundScan($type, array $attribute)
    {
        ...
        if ($canScan) {
            ...
            $jobId = $JobsTable->createJob(
                ...
                Job::WORKER_PRIO,
                'virus_scan',
                ($type === AttachmentScan::TYPE_ATTRIBUTE ? 'Attribute: ' : 'Shadow attribute: ') . $attribute['id'],
                'Scanning...'
            );
    ...
    private function scanAttachment($type, array $attribute, array $moduleInfo)
    {
        ...
        if ($type === AttachmentScan::TYPE_ATTRIBUTE) {
    ...
    private function extractInfoFromModuleResult(array $results)
    {
        ...
        foreach ($results['Object'] as $object) {
            ...
            if (in_array($object['template_uuid'], AttachmentScan::SIGNATURE_TEMPLATES, true)) {
                $software = null;
                $signatures = array();
    ...
    {
        if (!$this->moduleModel) {
            $this->moduleModel = $this->fetchTable('Modules');
        }
    ...
    {
        if (!in_array($type, [AttachmentScan::TYPE_ATTRIBUTE, AttachmentScan::TYPE_SHADOW_ATTRIBUTE], true)) {
