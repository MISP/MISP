...
class TemplateElementsController extends AppController
{
        ...
        if ($this->request->is('get')) {
            ...
            if ($this->TemplateElement->save($templateElement)) {
                ...
                if ($this->TemplateElement->$ModelType->save($this->request->data)) {
                    return new JsonResponse(array('saved' => true, 'success' => 'Element successfully added to template.'));
                } else {
            ...
            return new JsonResponse(array('saved' => false, 'errors' => $errorMessage));
        }
    }
    ...
    public function edit($type, $id)
    {
        ...
        if ($this->request->is('get')) {
            ...
            if ($this->TemplateElement->$ModelType->save($this->request->data)) {
                return new JsonResponse(array('saved' => true, 'success' => 'Element successfully edited.'));
            } else {
                $this->TemplateElement->delete($this->TemplateElement->id);
                $errorMessage = $this->TemplateElement->$ModelType->validationErrors;
            }
            return new JsonResponse(array('saved' => false, 'errors' => 'The element could not be edited.'));
        }
    }
        ...
        if ($this->request->is('post')) {
            if ($this->_isSiteAdmin() || $this->Auth->user('Organisation')['name'] == $this->TemplateElement->data['Template']['org']) {
                ...
                if ($this->TemplateElement->$type->delete($this->TemplateElement->data[$type][0]['id'])) {
                    ...
                    return new JsonResponse(array('saved' => true, 'success' => 'Element deleted.'));
                } else {
                    return new JsonResponse(array('saved' => true, 'errors' => 'Couldn\'t delete the Element'));
                }
            } else {
                return new JsonResponse(array('saved' => true, 'errors' => 'You don\'t have permission to do that.'));
            }
        } else {
            $this->set('id', $id);
