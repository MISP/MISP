...
class Module extends AppModel {
	...
	public function queryModuleServer($uri, $post = false, $hover = false, $moduleFamily = 'Enrichment', &$exception = false) {
		...
		if (!$url) return false;
		if ($hover) {
			$httpSocket = new Client(array('timeout' => Configure::read('Plugin.' . $moduleFamily . '_hover_timeout') ? Configure::read('Plugin.' . $moduleFamily . '_hover_timeout') : 5));
		} else {
			$httpSocket = new Client(array('timeout' => Configure::read('Plugin.' . $moduleFamily . '_timeout') ? Configure::read('Plugin.' . $moduleFamily . '_timeout') : 10));
		}
		$request = array(
				'header' => array(
