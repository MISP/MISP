...
| `char.png` | **"Char" skin** — an easter egg (DD-49) for the `webgl-globe` mode: a charred / molten-planet surface (glowing lava on a scorched globe). **User-supplied** PNG, 1774×887 (2:1 equirectangular — wraps correctly; NPOT but fine for the clamp-to-edge sphere texture). **Much larger than the other skins (2.9 MB)** — it is lazy-loaded (fetched only when "Char" is selected) so it costs nothing otherwise; could be downscaled to 2048×1024 + JPEG (~300 KB) if size matters. Not from `three-globe`. | 2.9 MB / — |
| `planet_nacre.png` | **"Planet Nacre" skin** — an easter egg (DD-49) for the `webgl-globe` mode: a pastel / pearlescent ("nacre") stylised planet. **User-supplied** PNG, 1774×887 (2:1 equirectangular; NPOT but fine). **2.7 MB** — like `char.png`, far larger than the NASA skins; lazy-loaded so only its pickers pay; downscale-to-JPEG (~300 KB) available on request. Not from `three-globe`. | 2.7 MB / — |
