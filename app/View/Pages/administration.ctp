<div class="index">
    ...
    <ul>
        ...
        <li><?php echo $this->Form->postLink(__('Recorrelate attributes'), $baseurl . '/attributes/generateCorrelation');?></li>
        <li><?php echo $this->Form->postLink(__('Recorrelate proposals'), $baseurl . '/shadow_attributes/generateCorrelation');?></li>
        ...
        <li><?php echo $this->Form->postLink(__('Convert log fields to text'), $baseurl . '/servers/updateDatabase/convertLogFieldsToText');?> (<?php echo __('Hotfix 2.3.78: Some of the log fields that were varchar(255) ended up truncating the data. This function will change them to "text"');?>)</li>
        ...
        <li><?php echo $this->Form->postLink(__('Remove duplicate events (with the same UUID)'), $baseurl . '/servers/removeDuplicateEvents');?> (<?php echo __('Hotfix 2.3.115: In some rare situations it could occur that a duplicate of an event was created on an instance, with the exact same uuid. This action will remove any such duplicates and make sure that this cannot happen again.');?>)</li>
        <li><?php echo $this->Form->postLink(__('Prune orphaned attributes'), $baseurl . '/attributes/pruneOrphanedAttributes');?> (<?php echo __('In some rare occasions it can happen that you end up with some attributes in your database that do not belong to an event - for example during a race condition between an event insert and a delete. This tool will collect and delete any such orphaned attributes. If you ever run into an issue where you cannot add an attribute with a specific valid value, this is probably the reason.');?>)</li>
        <li><?php echo $this->Form->postLink(__('Clean regex table of potentially malicious entries'), $baseurl . '/regexp/cleanRegexModifiers');?> (<?php echo __('Hotfix 2.3.160: Prior to this version it was possible for a user/admin with Regex permission to create a malicious regular expression that could be used to execute arbitrary code. Since this version it is no longer possible to input such expressions, but already existing malicious entries still have to be cleaned using this tool.');?>)</li>
        <li><?php echo $this->Form->postLink(__('Remove URL type attribute sanitisation'), $baseurl . '/attributes/updateAttributeValues/urlSanitisation');?> (<?php echo __('Hotfix 2.3.173: Sanitised URLs can cause issues with the NIDS exports and as of this version attributes will be modified on entry to correct this. To correct existing entries, run this script.');?>)</li>
        <li><?php echo $this->Form->postLink(__('Index tables'), $baseurl . '/servers/updateDatabase/indexTables');?> (<?php echo __('This script will create indices for all of the tables in MISP (other than primary keys)');?>)</li>
