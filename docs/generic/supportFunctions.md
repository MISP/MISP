...
usage () {
  ...
  echo -e "${HIDDEN}       -f | Force test install on current Ubuntu LTS shim, add -B for 18.04 -> 18.10, or -BB 18.10 -> 19.10)${NC}" # FORCE
...
checkInstaller () {
  ...
  if [[ "${chsum}" == "${INSTsum}" ]]; then
    ...
    # SHA sums to be computed, note the -- notation is for ease of use with rhash
