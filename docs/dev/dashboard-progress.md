...
- [ ] Widget gallery — metadata endpoint: `GET /dashboards/widgets` (new) extending `Dashboard::loadAllWidgets` / `__extractMeta` to include `$schema`, `$category`, `$thumbnail` per PRD §5.7; existing `checkPermissions` filter preserved; JSON wire shape mirrors PRD §5.8
- [ ] Widget gallery — views + CSS: `Elements/dashboard/gallery/grid.ctp` + `card.ctp` + Overmind themed mirror; category grouping; search box; static-thumbnail support via `$thumbnail`; gallery layout CSS in `dashboard.default.css`
- [ ] Widget gallery — side-panel open from "+ Add widget" (browse-only): wire the new toolbar button + JS fetch of `/dashboards/widgets`; gallery rendered inside `.misp-configure-panel` as a new view mode; close via existing ✕ / backdrop / ESC chain
- [ ] Add Widget flow — card click → schema form opens in side panel for a draft widget instance; new tile staged in the edit-mode snapshot (depends on the browse-only task above being closed)
- [ ] Add Widget flow — placement: Place inserts the draft via `Grid.addTile()` at the next free auto-place slot; Cancel discards the draft cleanly; new-tile inheritance from the toolbar's current canonical-typed values (PRD §5.6.4) wired at this step
- [ ] Add Widget flow — live preview on right: draft widget body re-rendered via `_renderWidget` POST against the in-memory draft config; consider closing the standalone "Sticky preview pane in configure side-panel" Phase 2 task in the same chunk (the two requirements overlap)
