...
pymisp_pyproject = 'PyMISP/pyproject.toml'
...
with open(pymisp_pyproject) as f:
    pymisp_version = re.findall('version = "(.*)"', f.read())[0]

if not controller_version == pymisp_version:
    raise Exception(f'PyMISP in {pymisp_pyproject} ({pymisp_version}) differs from the one in {app_controller} ({controller_version})')
