...
class ShadowAttributesController extends AppController {
	...
	public function accept($id = null) {
		...
		if ($this->_isRest()) {
			...
			return new Response(array('body'=> json_encode($response),'status'=>200));
		}
	}
		...
		if ($this->request->is('post')) {
			if ($this->__discard($id)) {
				if ($this->_isRest()) {
					...
					$this->autoRender = false;
					return new Response(array('body'=> json_encode(array('saved' => true, 'success' => 'Proposal discarded.')),'status'=>200));
				}
			} else {
				if ($this->_isRest()) {
					...
					$this->autoRender = false;
					return new Response(array('body'=> json_encode(array('false' => true, 'errors' => 'Could not discard proposal.')),'status'=>200));
				}
			}
		} else {
		...
		if ($this->request->is('post')) {
			...
			if (!$this->_isRest() && (isset($this->request->data['ShadowAttribute']['batch_import']) && $this->request->data['ShadowAttribute']['batch_import'] == 1)) {
				...
				if ($this->ShadowAttribute->save($this->request->data)) {
					...
					if ($this->request->is('ajax')) {
						$this->autoRender = false;
						return new Response(array('body'=> json_encode(array('saved' => true, 'success' => 'Proposal added' . $emailResult)),'status'=>200));
					...
					if ($this->request->is('ajax')) {
						$this->autoRender = false;
						return new Response(array('body'=> json_encode(array('saved' => false, 'errors' => $this->ShadowAttribute->validationErrors)),'status'=>200));
					} else if ($this->_isRest()) {
						$message = '';
	...
	public function delete($id) {
		...
		if ($this->request->is('post')) {
			if (empty($existingAttribute)) return new Response(array('body'=> json_encode(array('false' => true, 'errors' => 'Invalid Attribute.')),'status'=>200));
			$this->ShadowAttribute->create();
			$sa = array(
			...
			if ($this->ShadowAttribute->save($sa)) {
				...
				return new Response(array('body'=> json_encode(array('saved' => true, 'success' => 'The proposal to delete the attribute has been saved' . $emailResult)),'status'=>200));
			} else {
				return new Response(array('body'=> json_encode(array('false' => true, 'errors' => 'Could not create proposal.')),'status'=>200));
			}
		} else {
	...
	public function editField($id) {
		...
		if ($this->ShadowAttribute->save($proposal)) {
			...
			$this->autoRender = false;
			return new Response(array('body'=> json_encode(array('fail' => false, 'errors' => $this->ShadowAttribute->validationErrors)),'status'=>200));
		}
	}
	...
	public function discardSelected($id) {
		...
		if (!$this->_isSiteAdmin()) {
			...
			if ($event['Event']['orgc_id'] != $this->Auth->user('org_id') || (!$this->userRole['perm_modify_org'] && !($this->userRole['perm_modify'] && $event['Event']['user_id'] == $this->Auth->user('id')))) {
				return new Response(array('body'=> json_encode(array('false' => true, 'errors' => 'You don\'t have permission to do that.')),'status'=>200));
			}
		}
		...
		if (count($fails) == 0 && count($successes) > 0) {
			return new Response(array('body'=> json_encode(array('saved' => true, 'success' => count($successes) . ' proposal' . (count($successes) != 1 ? 's' : '') . ' deleted.')),'status'=>200));
		} else {
			return new Response(array('body'=> json_encode(array('saved' => false, 'errors' => count($successes) . ' proposal' . (count($successes) != 1 ? 's' : '') . ' deleted, but ' . count($fails) . ' proposal' . (count($fails) != 1 ? 's' : '') . ' could not be deleted.')),'status'=>200));
		}
	}
	...
	public function acceptSelected($id) {
		...
		$this->autoRender = false;
		if (count($fails) == 0 && count($successes) > 0) {
			return new Response(array('body'=> json_encode(array('saved' => true, 'success' => count($successes) . ' proposal' . (count($successes) != 1 ? 's' : '') . ' accepted.')),'status'=>200));
		} else {
			return new Response(array('body'=> json_encode(array('saved' => false, 'errors' => count($successes) . ' proposal' . (count($successes) != 1 ? 's' : '') . ' accepted, but ' . count($fails) . ' proposal' . (count($fails) != 1 ? 's' : '') . ' could not be accepted.')),'status'=>200));
		}
	}
