...
App::uses('AppHelper', 'View/Helper');
	...
	class PivotHelper extends AppHelper {
		...
		private function __doConvert($pivot, $currentEvent, $activeText=false) {
			...
			if ($pivot['deletable']) {
				$data[] = '<a class="pivotDelete icon-remove" href="'.$baseurl.'/events/removePivot/' . $pivot['id'] . '/' . $currentEvent . '"></a>';
			}
			$data[] = '<a class="' . $pivotType . '" href="'.$baseurl.'/events/view/' . $pivot['id'] . '/1/' . $currentEvent . '" title="' . h($pivot['info']) . ' (' . $pivot['date'] . ')">' . h($text) . '</a>';
			$data[] = '</span>';
