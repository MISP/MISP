...
class TemplateElementsController extends AppController {
	...
	public function add($type, $id) {
		...
		if (!$this->_isSiteAdmin() && !$this->TemplateElement->Template->checkAuthorisation($id, $this->Auth->user(), true)) return new Response(array('body'=> json_encode(array('saved' => false, 'errors' => 'You are not authorised to do that.')), 'status' => 200));
		...
		if ($this->request->is('get')) {
			...
			if ($this->TemplateElement->save($templateElement)) {
				...
				if ($this->TemplateElement->$ModelType->save($this->request->data)) {
					return new Response(array('body'=> json_encode(array('saved' => true, 'success' => 'Element successfully added to template.')), 'status' => 200));
				} else {
	...
	public function edit($type, $id) {
		...
		if ($this->request->is('get')) {
			...
			if ($this->TemplateElement->$ModelType->save($this->request->data)) {
				return new Response(array('body'=> json_encode(array('saved' => true, 'success' => 'Element successfully edited.')), 'status' => 200));
			} else {
				$this->TemplateElement->delete($this->TemplateElement->id);
				$errorMessage = $this->TemplateElement->$ModelType->validationErrors;
			}
			return new Response(array('body'=> json_encode(array('saved' => false, 'errors' => 'The element could not be edited.')), 'status' => 200));
		}
	}
		...
		if ($this->request->is('post')) {
			if ($this->_isSiteAdmin() || $this->Auth->user('Organisation')['name'] == $this->TemplateElement->data['Template']['org']) {
				...
				if ($this->TemplateElement->$type->delete($this->TemplateElement->data[$type][0]['id'])) {
					...
					return new Response(array('body'=> json_encode(array('saved' => true, 'success' => 'Element deleted.')), 'status' => 200));
				} else {
					return new Response(array('body'=> json_encode(array('saved' => true, 'errors' => 'Couldn\'t delete the Element')), 'status' => 200));
				}
			} else {
				return new Response(array('body'=> json_encode(array('saved' => true, 'errors' => 'You don\'t have permission to do that.')), 'status' => 200));
			}
		} else {
			$this->set('id', $id);
