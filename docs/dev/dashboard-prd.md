...
|---|---|---|---|---|
| DD-51 | 2026-05-30 | **Supersedes §8.1 / `project-misp-dark-theme-sequencing` for the interim** (which deferred dashboard dark mode to a *global* MISP theme — empty risk while no such theme exists; half-measure accepted as stopgap), DD-13 (`-muted` tokens are the choropleth low-stops it completes for dark), DD-47 G6 (the globe's token-driven retheme bridge it reuses), `feedback_additive_only_posture` (the server-side persistence the user explicitly signed off on crosses additive-only — a new UserSetting key + one controller action), `feedback_verify_visible_outcome_not_property` (verified the retoned screenshot + persisted round-trip, not the attribute set) | **Stopgap dashboard-local light/dark toggle.** Per-user toggle on the dashboard until a global MISP dark theme ships (user asked for it "for the meanwhile"). Persistence = **server-side** `UserSetting:dashboard_theme` (`auto`\|`light`\|`dark`; `auto` = follow OS `prefers-color-scheme`, never written back); first-visit = **follow-OS**; interaction = **live retheme, no reload**. A no-FOUC inline `<head>` script (`theme_boot.ctp`, in all 3 dashboard layouts) seeds `data-theme` from the pref before first paint; a sun/moon icon button (`index.ctp`, beside the refresh toggle) flips it live via `board.module.mjs::_toggleTheme` + persists via `updateTheme`. CSS chrome + the globe self-retheme; ECharts is rebuilt by a new `rethemeChartsIn()` (force re-register + re-init from the unchanged DOM payload; globe skipped). Completed the dark overlay: dark-tuned `-muted` tokens + scoped fixups for two black overlays in `dashboard.midnight.css`, and tokenised the red washes / a card shadow in `dashboard.default.css`. Verified: curl round-trip (persist + boot-script read-back + 400 on bad value), headless light/dark screenshots (button glyph swap + live chart retone), lint + 77 PHPUnit green. Reverse = drop the button/boot/action/endpoint/key; the now token-complete midnight overlay is what a future global dark theme reuses. | binding |
| DD-50 | 2026-05-30 | DD-47 (extends its `webgl-globe` mode — idle motion on the same globe.gl instance; no server/payload change), DD-46/DD-47 ("auto-rotate deferred for screenshot stability" is the precedent this lifts), `feedback_additive_only_posture` (pure front-end addition inside the feature's own scope), `feedback_verify_visible_outcome_not_property` (verified the rendered rotation, not the IDL property) | **Slow idle auto-rotate for the WebGL globe ("Globe (3D)").** Polish: the opt-in `webgl-globe` mode now spins gently when idle (a slow attract-mode spin suiting the playful pew-pew vibe), deferred in DD-47 v1 for screenshot stability. globe.gl's default controls are OrbitControls (`controlType:'orbit'`) and its render loop already ticks `controls.update()` each frame, so this is a plain property set — `controls.autoRotate = true` + `autoRotateSpeed = 0.6` (~100 s/revolution) in `initWebglGlobe`; no extra ticker, no new config field. **Gated on `prefers-reduced-motion`** — a user who asks for less motion gets a static (still draggable) globe; a drag transiently overrides the spin and OrbitControls resumes it on release. Front-end only; `handler()`/`flows[]`/caching + the 2d/3d-globe modes unchanged. Verified headless (swiftshader): an empty-flows globe rotates tens of degrees over a large virtual-time gap (compare AE 41790 px) while a `--force-prefers-reduced-motion` capture is byte-identical (AE 0); DOM probe confirms the real WebGL path (canvas 600×420, lazy bundle + night texture fetched, no fallback). Reverse = drop the `autoRotate`/`autoRotateSpeed` lines + the reduced-motion guard; the globe goes back to static. | binding |
| DD-49 | 2026-05-29 | DD-47 (extends its `webgl-globe` mode; same lazy-texture / front-end-only posture), DD-46 D4 (`enum_labels` mechanism), DD-07 (NASA imagery is public domain — no licence review), `feedback_additive_only_posture` (pure addition — a new optional config field + two vendored textures) | **Selectable globe skins (night / day / dark) for the WebGL globe.** User: the default night texture is "pretty dark", wants a daytime version. Add a per-widget `skin` config on the `webgl-globe` mode (DD-47): **`night`** (default, city-lights Black Marble), **`day`** (NASA Blue Marble daytime earth), **`dark`** (minimal near-black grey) — plus two later user-supplied easter-egg skins **`char`** (charred/molten) + **`nacre`** ("Planet Nacre", pearlescent), ~2.7–2.9 MB PNGs, lazy. Front-end only — texture is just a different `globeImageUrl`; `handler()`/`flows[]` (now carries a `skin` hint)/caching + the 2d/3d-globe modes unchanged; those modes ignore `skin`. Only the selected skin's image is fetched (lazy, per instance). Textures are NASA PD from `three-globe`'s example dir, downscaled 2048×1024 (`earth-{night,day,dark}-2k.jpg`; 205/279/81 KB). **Gotcha (= DD-47 G5):** a skin must be listed in 3 places — the `$schema` enum, `resolveSkin()` whitelist, AND the `GLOBE_TEXTURES` map — or it degrades to `night`. Verified: 15/15 tests, live renders night/day/dark (+ bogus→night), headless-Chrome shots of day + dark render correctly. Reverse = drop the enum/param/resolveSkin + map entries + the two textures; night skin stands alone. | binding |
