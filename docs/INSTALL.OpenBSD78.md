...
## for OpenBSD 7.8-amd64

!!! notice 
        Updated and tested by [@tomiodarim](https://github.com/tomiodarim) on 2025-11-28.
...
```bash
doas syspatch
# Sometimes needs to be run a second time to install everything
doas syspatch
```
...
```bash
doas pkg_add -v git sqlite3 python--%3.12 redis libmagic autoconf--%2.71 automake--%1.16 libtool unzip--iconv rust
```
...
doas ln -sf /usr/local/bin/pip3.12 /usr/local/bin/pip
doas ln -s /usr/local/bin/python3.12 /usr/local/bin/python
...
#### php8.4 ports
```
doas pkg_add -v php-mysqli--%8.4 php-pcntl--%8.4 php-pdo_mysql--%8.4 php-apache--%8.4 pecl84-redis php-gd--%8.4 php-zip--%8.4 php-intl--%8.4 php-curl--%8.4
```

#### /etc/php-8.4.ini 
```
doas sed -i "s/^allow_url_fopen = Off/allow_url_fopen = On/g" /etc/php-8.4.ini
```

```bash
cd /etc/php-8.4
doas cp ../php-8.4.sample/* .
```

#### php symlinks
```bash
doas ln -s /usr/local/bin/php-8.4 /usr/local/bin/php
doas ln -s /usr/local/bin/phpize-8.4 /usr/local/bin/phpize
doas ln -s /usr/local/bin/php-config-8.4 /usr/local/bin/php-config
```
...
doas pkg_add -v libxslt py3-jsonschema
doas /usr/local/virtualenvs/MISP/bin/pip install -U pip setuptools setuptools-rust 
doas /usr/local/virtualenvs/MISP/bin/pip install lxml stix2 ordered_set pycountry
...
doas /usr/local/virtualenvs/MISP/bin/pip install .
...
doas sed -i "s|define('CAKE_CORE_INCLUDE_PATH', ROOT . DS . APP_DIR . DS . 'Lib' . DS . 'cakephp' . DS . 'lib');|define('CAKE_CORE_INCLUDE_PATH', '/htdocs/MISP/app/Lib/cakephp/lib');|" /var/www/htdocs/MISP/app/webroot/index.php

doas mkdir -p /htdocs
doas ln -s /var/www/htdocs/MISP /htdocs/
```
...
# You need to insert manually the password, don't use the variable
...
doas vi /etc/apache2/sites-available/misp-ssl.conf
```
...
```bash
doas ln -sf /var/www/conf/modules.sample/php-8.4.conf /var/www/conf/modules/php.conf
# Restart apache
...
doas gpg --homedir /var/www/htdocs/MISP/.gnupg --batch --gen-key /tmp/gen-key-script
# The email address should match the one set in the config.php / set in the configuration menu in the administration menu configuration file

# And export the public key to the webroot
doas sh -c "gpg --homedir /var/www/htdocs/MISP/.gnupg --export --armor $GPG_EMAIL_ADDRESS > /var/www/htdocs/MISP/app/webroot/gpg.asc"
