		...
		public function convertQuotes($string){
			...
			while (preg_match ('%\[event\](.*?)\[/event\]%is', $string, $matches)) {
				if (!empty($matches) && is_numeric($matches[1])) {
					$string = preg_replace('%\[event\]' . $matches[1] . '\[/event\]%i', '<a href='.$baseurl.'/events/view/' . $matches[1] . '> Event ' . $matches[1] . '</a>', $string);
				} else {
			...
			while (preg_match ('%\[thread\](.*?)\[/thread\]%is', $string, $matches)) {
				if (!empty($matches) && is_numeric($matches[1])) {
					$string = preg_replace('%\[thread\]' . $matches[1] . '\[/thread\]%i', '<a href='.$baseurl.'/threads/view/' . $matches[1] . '> Thread ' . $matches[1] . '</a>', $string);
				} else {
