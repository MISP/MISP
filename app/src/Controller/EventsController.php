...
class EventsController extends AppController {
					...
					if ($add === true && !is_numeric($add)) {
						if ($this->_isRest()) {
							...
							if (!Session::read('Message.flash')) {
								...
								$existingFlash = Session::read('Message.flash');
						...
						if ($this->_isRest()) { // TODO return error if REST
							if (is_numeric($add)) {
								...
								$this->response->send();
								throw new NotFoundException('Event already exists, if you would like to edit it,							}
	...
	public function hids($type, $key = 'download', $tags = false, $from = false, $to = false, $last = false, $enforceWarninglist = false) {
		...
		return new Response(array('body'=> implode(PHP_EOL, $rules), 'status' => 200, 'type' => 'txt'));
	}
	...
	public function downloadSearchResult() {
		...
		/* TODO: App::uses(strtoupper($type) . 'ConverterTool', 'Tools'); */
	...
	public function restSearch($key = 'download', $value = false, $type = false, $category = false, $org = false, $tags = false, $searchall = false, $from = false, $to = false, $last = false, $eventid = false, $withAttachments = false, $metadata = false, $uuid = false, $publish_timestamp = false, $timestamp = false, $published = false, $enforceWarninglist = false, $sgReferenceOnly = false) {
		...
		/* TODO: App::uses($converters[$responseType], 'Tools'); */
	...
	public function addTag($id = false, $tag_id = false) {
		...
		$this->autoRender = false;
		if (!empty($found)) return new Response(array('body'=> json_encode(array('saved' => false, 'errors' => 'Tag is already attached to this event.')), 'status'=>200));
		...
		if ($this->Event->EventTag->save(array('event_id' => $id, 'tag_id' => $tag_id))) {
			...
			return new Response(array('body'=> json_encode(array('saved' => true, 'success' => 'Tag added.', 'check_publish' => true)), 'status'=>200));
		} else {
			return new Response(array('body'=> json_encode(array('saved' => false, 'errors' => 'Tag could not be added.')),'status'=>200));
		}
	}
	...
	public function removeTag($id = false, $tag_id = false, $galaxy = false) {
		if (!$this->request->is('post')) {
			...
			if ($id === false) $id = $this->request->data['event'];
			if ($tag_id === false) $tag_id = $this->request->data['tag'];
			if (empty($tag_id)) return new Response(array('body'=> json_encode(array('saved' => false, 'errors' => 'Invalid ' . ($galaxy ? 'Galaxy' : 'Tag') . '.')),'status'=>200));
			if (!is_numeric($tag_id)) {
				$tag = $this->Event->EventTag->Tag->find('first', array('recursive' => -1, 'conditions' => array('LOWER(Tag.name) LIKE' => strtolower(trim($tag_id)))));
				if (empty($tag)) return new Response(array('body'=> json_encode(array('saved' => false, 'errors' => 'Invalid ' . ($galaxy ? 'Galaxy' : 'Tag') . '.')), 'status'=>200));
				$tag_id = $tag['Tag']['id'];
			}
			...
			$this->autoRender = false;
			if (empty($eventTag)) return new Response(array('body'=> json_encode(array('saved' => false, 'errors' => 'Invalid event - ' . ($galaxy ? 'galaxy' : 'tag') . ' combination.')),'status'=>200));
			$tag = $this->Event->EventTag->Tag->find('first', array(
				'conditions' => array('Tag.id' => $tag_id),
				'recursive' => -1,
				'fields' => array('Tag.name')
			...
			if ($this->Event->EventTag->delete($eventTag['EventTag']['id'])) {
				...
				return new Response(array('body'=> json_encode(array('saved' => true, 'success' => ($galaxy ? 'Galaxy' : 'Tag') . ' removed.', 'check_publish' => true)), 'status'=>200));
			} else {
				return new Response(array('body'=> json_encode(array('saved' => false, 'errors' => ($galaxy ? 'Galaxy' : 'Tag') . ' could not be removed.')),'status'=>200));
			}
		}
	}
	...
	public function updateGraph($id) {
		...
		return new Response(array('body' => json_encode($json), 'status' => 200));
	}
	...
	public function checkPublishedStatus($id) {
		...
		return new Response(array('body'=> h($event[0]['Event']['published']), 'status'=>200));
	}
