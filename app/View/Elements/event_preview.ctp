<div class="events view">
	<?php
		...
		if ($context == 'feed') {
			$warning = 'You are currently previewing an event from a feed (' . h($feed['Feed']['name']) . ' by ' . h($feed['Feed']['provider']) . ')';
			$id = $feed['Feed']['id'];
			$eventId = $event['Event']['uuid'];
		} elseif ($context == 'server') {
			$warning = 'You are currently viewing an event on the remote instance ' . h($server['Server']['name']) . ' (@ ' . h($server['Server']['url']) . ')';
			$id = $server['Server']['id'];
			$eventId = $event['Event']['id'];
		}
	?>
	<h4 class="visibleDL notPublished" ><?php echo $warning; ?></h4>
	<div class="row-fluid">
		<div class="span8">
				...
				<?php if (Configure::read('MISP.tagging')): ?>
					...
					<?php if (!empty($event['Tag'])) foreach ($event['Tag'] as $tag): ?>
						<span style="padding-right:0px;">
							<a href="/<?php echo h(ucfirst($context)) . 's'; ?>/previewIndex/<?php echo h($id); ?>/searchtag:<?php echo h($tag['name']); ?>" class="tagFirstHalf" style="background-color:#00ffcf;color:<?php echo h($tag['colour']); ?>"><?php echo h($tag['name']); ?></a>
						</span>
					<?php endforeach; ?>&nbsp;
					</dd>
				<?php endif; ?>
	...
	<div class="related span4">
		...
		<ul class="inline">
			...
			<a href = "<?php echo '/' . h(ucfirst($context)) . 's' . '/previewEvent/' . $id . '/' . $relatedEvent['Event'][0]['id']; ?>"><?php echo h($relatedEvent['Event'][0]['date']) . ' (' . h($relatedEvent['Event'][0]['id']) . ')'; ?></a>
			</div></li>
			<?php endforeach; ?>
		</ul>
	</div>
				...
				?>
					<tr id = "<?php echo $currentType . '_' . $object['id'] . '_tr'; ?>" class="<?php echo $extra3; ?>">
						...
						<td class="shortish <?php echo $extra; ?>">
							...
							?>
										<li style="padding-right: 0px; padding-left:0px;" title ="' . h($relatedAttribute['info']) . '"><span><a href="/<?php echo h(ucfirst($context)); ?>/previewEvent/<?php echo h($id); ?>/<?php echo h($event['Event']['id']); ?>"><?php echo h($event['Event']['id']); ?></a></span>
							<?php
									endforeach;
								endif; 
							?>
